...
const ItemManager = {
  ...
  update(skuCode, itemInformation) {
    const item = this.getItem(skuCode);

    Object.keys(itemInformation).forEach(key => {
      item[key] = itemInformation[key];
    });
  },

  delete(skuCode) {
    const deleteIndex = this.items.findIndex(item => item.skuCode === skuCode);
  ...
  itemsInCategory(category) {
    console.log(this.items.filter(item => item.category === category));
  },
};
...
// logs list with the 4 valid items
