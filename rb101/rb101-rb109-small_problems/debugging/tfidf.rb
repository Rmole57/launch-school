...
# The problem was in the #idf method. You were dividing integers in
# the Math.log() method call and there were many times where, because
# of integer division, you weren't getting a precise calculation,
# leading Math.log() to rightfully return 0. 
...
# If you remove document1 from documents then number_of_documents_with_term
# will always be 0, and in floating-point division, dividing a non-zero
# number by 0 will give us Infinity. 

# Continuing through to #tfidf will give us Infinty when using document1
# in #tf: positive_number * Infinity, or NaN when using the other documents:
# 0 * Infinity.

# There are a couple of ways to fix this...

# You could default the return value of #idf to 0.0 if the return value
# of the call to Math.log() is Infinity or NaN:

# def idf(term, documents)
#   number_of_documents = documents.length
#   number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }
#   idf = Math.log(number_of_documents.to_f / number_of_documents_with_term.to_f)
#   if idf.infinite? || idf.nan?
#     0.0
#   else
#     idf
#   end
...
# Or, and perhaps a more user-friendly way, is to display an error message
# in the #tfidf method if the return value is Infinity or NaN:

# def tfidf(term, document, documents)
#   tfdif = (tf(term, document) * idf(term, documents)).round(1)
#   if tfdif.infinite? || tfdif.nan?
#     "The word was not found in any of the documents in the 'documents' variable."
#   else
#     tfdif
#   end
# end
