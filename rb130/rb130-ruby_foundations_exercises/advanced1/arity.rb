...
# 3. A Proc object does not require that the correct number of arguments
#    are passed to it. If nothing is passed, then nil is assigned to the
#    block variable.
...
#    This can be seen when displaying a Lambda: the string displayed contains an extra
#    "(lambda)" that is not present for regular Procs.
...
# 1. A block passed to a method does not require the correct number of arguments.
#    If a block variable is defined, and no value is passed to it, then nil will
#    be assigned to that block variable.
...
# 1. If we pass too few arguments to a block, then the remaining ones
#    are assigned a nil value.
# 2. Blocks will throw an error if a variable is referenced that doesn't
...
#    do not enforce the number of arguments passed in.
