...
function getStudentScore(scoreObj) {
  ...
  let examAverage = computeExamsAverage(scoreObj.exams) * EXAM_WEIGHT;
  let exerciseAverage = computeScoreTotal(scoreObj.exercises) * EXERCISE_WEIGHT;

  let numberGrade = Math.round(examAverage + exerciseAverage);

  return `${numberGrade} (${getLetterGrade(numberGrade)})`;
}

function getExamSummary(examScoresPerStudent) {
  let scoresPerExam = [];

  examScoresPerStudent.forEach(scores => {
    scores.forEach((score, index) => {
      scoresPerExam[index] = scoresPerExam[index] || [];
      scoresPerExam[index].push(score);
    });
  });

  return scoresPerExam.map(scores => {
    let average = Number(computeExamsAverage(scores).toFixed(1));
...
function computeExamsAverage(exams) {
  return computeScoreTotal(exams) / exams.length;
}
